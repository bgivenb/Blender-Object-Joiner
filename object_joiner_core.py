...
class MeshSummary:
    ...
    def change_percent(self) -> float:
        if self.source_polygons == 0:
            return 0.0
        return 100.0 * (self.result_polygons - self.source_polygons) / self.source_polygons


def validate_settings(mesh_count: int, voxel_size: float, target_detail: float) -> None:
    if mesh_count < 2:
        raise ValueError("Select at least two mesh objects.")
    if not 0.0001 <= voxel_size <= 1.0:
        raise ValueError("Voxel size must be between 0.0001 and 1.0 metres.")
